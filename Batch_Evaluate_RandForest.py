import pandas as pd
import joblib
from sklearn.metrics import accuracy_score, classification_report

# --------------------------
# 1. Load Saved Model & Features
# --------------------------
print("📥 Loading saved model and feature list...")
model = joblib.load("Random_Forest TRAINED_MODEL.pkl")
features = joblib.load("RandForest_model_features.pkl")
print("✅ Model and features loaded.")

# --------------------------
# 2. Load Full Dataset and Filter African URLs
# --------------------------
print("📄 Loading dataset...")
df = pd.read_csv("PhiUSIIL_Phishing_URL_Dataset.csv")
african_ccTLDs = ['.za', '.ng', '.ke', '.gh', '.tz', '.ug', '.bw', '.zm', '.zw', '.sn', '.cm', '.dz', '.ma', '.eg']
df = df[df['URL'].str.contains('|'.join(african_ccTLDs), na=False)]
print(f"🌍 Filtered African dataset shape: {df.shape}")

# --------------------------
# 3. Data Cleaning and Feature Prep
# --------------------------
columns_to_drop = ['FILENAME', 'URL', 'Domain', 'Title']
df = df.drop(columns=[col for col in columns_to_drop if col in df.columns], errors='ignore')
df = df.dropna()
df = df.select_dtypes(include=['number', 'bool'])

# Ensure the dataset has the required features
X = df[features]
y_true = df['label']

# --------------------------
# 4. Batch Prediction and Evaluation
# --------------------------
print("🔮 Running batch predictions...")
y_pred = model.predict(X)

print("✅ Prediction complete.\n")

print("🎯 Evaluation Results:")
print("Accuracy:", accuracy_score(y_true, y_pred))
print(classification_report(y_true, y_pred))

print("\n✅ Batch evaluation complete.")
